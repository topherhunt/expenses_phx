defmodule ExpensesWeb.AuthController do
  require Logger
  use ExpensesWeb, :controller
  alias Expenses.Data
  alias Expenses.Data.User
  alias ExpensesWeb.AuthPlugs
  ...
  def signup_submit(conn, %{"user" => user_params}) do
    case Data.insert_user(%User{}, user_params, :owner) do
      {:ok, user} ->
        Logger.info "#{__MODULE__}: Registered new user #{user.id}."
        Expenses.Emails.confirm_address(user, user.email) |> Expenses.Mailer.send()
  ...
  def login_submit(conn, %{"user" => %{"email" => email, "password" => password}}) do
    ...
    cond do
      ...
      true ->
        ...
        conn
        |> ExpensesWeb.AuthPlugs.login!(user)
  ...
  def confirm_email(conn, %{"token" => token}) do
    case Data.parse_token(token) do
      {:ok, {:confirm_email, user_id, email}} ->
        ...
        {:ok, _} = Data.update_user(user, attrs, :admin)
  ...
  def request_password_reset_submit(conn, %{"user" => %{"email" => email}}) do
    ...
    if user do
      Expenses.Emails.reset_password(user) |> Expenses.Mailer.send()
    end
